...
$(document).ready(function () {
  function setRobinhoodAmount() {
    if (jQuery(".OtherPropertyView:contains(Robinhood Account)").length) {
      console.log(
        "Robinhood Mint Integration - Updating Robinhood Portfolio Value"
      );
      var accountTotal = urlParams.get("portfolioAmount");

      var robinhoodContainer = jQuery(
        ".OtherPropertyView:contains(Robinhood Account)"
      );
      ...
      if (jQuery(robinhoodInputs[0]).val() == "Robinhood Account") {
        ...
        jQuery(
          "<script>jQuery('.saveButton').attr('disabled',false).click();</script>"
        ).appendTo("body");
